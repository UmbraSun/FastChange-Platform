        ...
        return Task.Run(() =>
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    var handler = scope.ServiceProvider.GetRequiredService<IEventHandler<TransactionCompletedEvent>>();
                    var @event = JsonSerializer.Deserialize<TransactionCompletedEvent>(result.Message.Value)!;
