...
public partial class DepositViewModel : ObservableObject
{
    ...
    private async Task DepositAsync()
    {
        ...
        try
        {
            ...
            var response = await _walletOperationsService.DepositAsync(SelectedWallet.WalletId, amount);
            var wallet = Wallets.FirstOrDefault(x => x.WalletId == response.WalletId);
            var depositedAmount = amount;

            if (wallet is not null)
            {
                var index = Wallets.IndexOf(wallet);
                Wallets[index] = wallet with { Balance = response.NewBalance };
                SelectedWallet = Wallets[index];
            }

            AmountText = string.Empty;
            SuccessMessage = $"Deposited {depositedAmount:N2} {SelectedWallet.Currency}.";
        }
        catch (Exception ex)
        {
            ErrorMessage = GetErrorMessage(ex);
    ...
    private static string GetErrorMessage(Exception exception)
    {
        return exception switch
        {
            ...
            ApiException { StatusCode: HttpStatusCode.Conflict } => "The deposit could not be completed.",
            HttpRequestException => "Unable to connect to the server.",
            _ => "Unable to complete the deposit."
        };
    }
}